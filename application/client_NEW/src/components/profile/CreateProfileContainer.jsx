...
export default class EditProfileContainer extends Component {
  ...
  toProfile() {
    if (!this.state.invalidValues) this.props.history.push("/profile");
  }
  ...
  saveProfile() {
    ...
    this.setState({
      invalidData: {
        picture: picture === "invalid" ? true : false,
    ...
    if (
      ...
      profile.append("profile_id", this.props.user.id);
      ...
      profile.append("user", this.props.user.id);
      profile.append("picture", picture);

      axios({
        method: "post",
        url: "http://www.localhost:8080/add-profile",
        data: profile,
